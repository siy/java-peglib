    ...
    public void put(long key, long value) {
        ...
        while (true) {
            ...
            if (s == EMPTY) {
                ...
                } else if (size + tombstones > threshold) {
                    // Tombstones alone are crowding the table — rehash at the
                    // current capacity to clear them and keep probe chains short.
                    // Without this, repeated put/remove cycles can saturate the
                    // table with TOMBSTONE+OCCUPIED slots and cause the put
                    // probe loop (which only stops at EMPTY) to spin forever.
                    resize(state.length);
                }
                return;
            }
