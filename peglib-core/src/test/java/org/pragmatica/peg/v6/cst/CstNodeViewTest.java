...
class CstNodeViewTest {
    ...
    private static final String[] TOKEN_NAMES = {
        "WHITESPACE", "LINE_COMMENT", "BLOCK_COMMENT", "NUMBER", "PLUS"
    };
    ...
    void viewAt_branchVariant_whenNodeHasChildren() {
        ...
        assertThat(view).isInstanceOf(CstNode.Branch.class);
        var branch = (CstNode.Branch) view;
        assertThat(branch.index()).isEqualTo(cst.rootIndex());
        assertThat(branch.array()).isSameAs(cst);
        assertThat(branch.kind()).isEqualTo(KIND_SUM);
        assertThat(branch.kindName()).isEqualTo("Sum");
        assertThat(branch.spanStart()).isZero();
        assertThat(branch.spanEnd()).isEqualTo(3);
        assertThat(branch.text().toString()).isEqualTo("1+2");
        assertThat(branch.children().boxed().toList()).containsExactly(1, 2, 3);
    }
    ...
    void viewAt_leafVariant_whenNodeHasNoChildrenAndNoErrorFlag() {
        ...
        assertThat(lhsView).isInstanceOf(CstNode.Leaf.class);
        var leaf = (CstNode.Leaf) lhsView;
        assertThat(leaf.index()).isEqualTo(1);
        assertThat(leaf.kind()).isEqualTo(KIND_NUMBER);
        assertThat(leaf.kindName()).isEqualTo("Number");
        assertThat(leaf.text().toString()).isEqualTo("1");
        assertThat(leaf.children().boxed().toList()).isEmpty();
    }
    ...
    void viewAt_errorVariant_whenErrorFlagSet() {
        ...
        assertThat(view).isInstanceOf(CstNode.Error.class);
        var err = (CstNode.Error) view;
        assertThat(err.index()).isEqualTo(cst.rootIndex());
        assertThat(err.kind()).isEqualTo(KIND_ERR);
        assertThat(err.kindName()).isEqualTo("Err");
        assertThat(err.text().toString()).isEqualTo("??");
    }
    ...
    void patternMatch_overSealedHierarchy_compilesAndDispatches() {
        ...
        assertThat(sumLabel).isEqualTo("branch:Sum");
        assertThat(leafLabel).isEqualTo("leaf:Number");

        var errCst = buildErrorCst();
        var errLabel = labelOf(errCst.viewAt(errCst.rootIndex()));
        assertThat(errLabel).isEqualTo("error:Err");
    }
    ...
    void allViewMethods_delegateToArray() {
        ...
        assertThat(view.array()).isSameAs(cst);
        assertThat(view.index()).isEqualTo(cst.rootIndex());
        assertThat(view.kind()).isEqualTo(cst.kindAt(cst.rootIndex()));
        assertThat(view.kindName()).isEqualTo(cst.kindNameAt(cst.rootIndex()));
        assertThat(view.spanStart()).isEqualTo(cst.spanStart(cst.rootIndex()));
        assertThat(view.spanEnd()).isEqualTo(cst.spanEnd(cst.rootIndex()));
        assertThat(view.text().toString()).isEqualTo(cst.textAt(cst.rootIndex()).toString());
    }
