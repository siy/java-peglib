...
/**
 * 0.6.1 — independent verification that the generated-parser path
 ...
 *
 * <p>The attribution walks tokens backward from a node's first content
...
class FirstMemberTriviaTest {
    ...
    @Test
    void firstMember_inheritsLineCommentAfterOpeningDelimiter() {
    ...
    @Test
    void firstMember_inheritsDocLineCommentAfterOpeningDelimiter() {
        var parser = PegParser.fromGrammar(GRAMMAR).unwrap();
        var input = "{ /// doc on first field\n  @x;\n}\n";
        var result = parser.parse(input);
