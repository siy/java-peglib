...
public final class LexerGenerator {
    ...
    private static String renderSource(String packageName,
        ...
        sb.append("    private ").append(className)
                 .append("() {}\n\n");
        sb.append("    public static final int STATE_COUNT = ").append(stateCount)
                 .append(";\n");
        sb.append("    public static final int ALPHABET_SIZE = ").append(alphabet)
                 .append(";\n");
        sb.append("    public static final int WHITESPACE_KIND = ").append(whitespaceKind)
                 .append(";\n\n");
    ...
    private static void renderResolvers(StringBuilder sb, DfaBuilder.TokenKindAssignment kinds) {
        ...
        sb.append("    private static final java.util.HashMap<String, Integer>[] RESOLVERS = new java.util.HashMap[").append(nameTableLen)
                 .append("];\n");
        ...
        for ( var entry : resolutions.entrySet()) {
            ...
            for ( var textEntry : entry.getValue().textToKind()
                                                .entrySet()) {
            sb.append("        ").append(local)
                     ...
                     .append(");\n");}
            sb.append("        RESOLVERS[").append(entry.getKey())
                     .append("] = ")
                     .append(local)
                     .append(";\n");
            idx++;
    ...
    private static void renderKindNames(StringBuilder sb, DfaBuilder.TokenKindAssignment kinds) {
        ...
        for ( int i = 0; i < names.length; i++) {
            if ( i > 0) {
            sb.append(", ");}
            sb.append('"').append(escapeJavaString(names[i]))
                     .append('"');
        }
        sb.append("};\n\n");
    }
    ...
    private static void renderTransitions(StringBuilder sb, int[][] transitions, int stateCount, int alphabet) {
        ...
        for ( int chunk = 0; chunk < chunkCount; chunk++) {
            ...
            sb.append("    private static void fillT").append(chunk)
                     .append("(int[] t) {\n");
